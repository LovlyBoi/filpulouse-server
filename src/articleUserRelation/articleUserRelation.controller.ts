...
class ArticleUserRelationController {
  ...
  save: Middleware = async (ctx,next) =>{
    ...
    if (await articleUserRelationService.save(ctx.tokenData.userId,body.articleId)){
  ...
  queryByArticleId : Middleware = async (ctx,next) =>{
    ...
    const r = await articleUserRelationService.getByArticleId(ctx.tokenData.userId,ctx.query['articleId']);
    ctx.body = {
      code:"200",
      msg:"success",
      data:r
  ...
  page: Middleware = async (ctx, next) => {
    ...
    const data = await articleUserRelationService.pageQuery({pageSize,pageNumber},ctx.query['articleId']);
...
export const articleUserRelationController = new ArticleUserRelationController();
